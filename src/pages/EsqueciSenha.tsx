...
export default function ForgotPassword() {
  ...
  const handleForgotPassword = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    navigate('/login/form') // Redireciona de volta para o login tradicional
  }
    ...
    <div className="min-h-screen w-full relative flex items-center justify-center bg-[#f4f7fe] dark:bg-[#0b1329] p-4 transition-colors duration-300">
      ...
      <div className="w-full max-w-md bg-white dark:bg-[#111c44] rounded-2xl shadow-xl shadow-gray-200/50 dark:shadow-none p-8 border border-gray-100 dark:border-[#1e2d60] transition-colors duration-300 relative">
        ...
        <form onSubmit={handleForgotPassword} className="space-y-5">
          <div>
            <label className="block text-sm font-semibold text-[#1b2559] dark:text-gray-300 mb-2">
              E-mail cadastrado
            </label>
            <input
              type="email"
              ...
              className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-[#1e2d60] bg-transparent text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-gray-400 dark:focus:border-[#4da1ff] transition-all"
            />
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
              Enviaremos um link seguro para você redefinir a sua senha atual.
            </p>
          </div>
